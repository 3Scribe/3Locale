...
## Milestone 5: Community automatic translation

Automatic translation is optional and uses the self-hoster's own DeepL API key. Community neither supplies credentials nor pays for usage; DeepL bills the owner's account. The rest of the product works without a key. Configuration and operational limits are in [DEPLOYMENT.md](DEPLOYMENT.md#optional-automatic-translation-byok).

For a selected target language, translate from the project's Base Language only. Choose all missing values or one missing entry. The preview is free of external calls and shows provider availability, supported language pair, eligible strings, skipped existing values and source Unicode code points. An explicit confirmation warns that text is sent to DeepL and can incur charges. No action silently translates during import, project loading, export, restoration or approval, and no nonblank target is overwritten.

Placeholders survive exactly, including repeated and mixed brace forms. The result identifies failed paths; valid individual results may be saved together while invalid results remain untouched. Provider failures and concurrent project changes discard the operation without partial writes, although completed provider requests may still be charged. Machine results visibly identify their provider and require review before normal JSON or ZIP export. Per-entry approval preserves text and machine provenance; bulk approval requires confirmation. Editing machine text changes provenance to manual and clears provider/model metadata. A later Base Language change makes existing targets review-required again while preserving their origins.

Automatic translation writes structured audit summaries and before/after recovery revisions using the existing newest-100 policy. Approval changes are audited without consuming full revisions. Unchanged entries keep their timestamps. The workflow is localised in English and Arabic and behaves identically on Node/SQLite and Workers/D1. DeepL is the sole production adapter in this milestone; no managed credentials, background queues, retranslation, translation memory, authentication, billing system or additional resource format is introduced.
