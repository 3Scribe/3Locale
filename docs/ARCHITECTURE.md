...
The portable `TranslationProvider` port describes synchronous offline capability/configuration checks and asynchronous translation of identified items. Requests carry source/target language tags, structural paths and opaque protected text; results preserve item IDs and optionally report real provider model/billing metadata. `MachineTranslationService` orchestrates selection, preview, confirmation, shielding, validation, optimistic concurrency and persistence. It imports no DeepL, HTTP, environment or database implementation. The DeepL adapter uses portable fetch and a strict XML parser; runtime composition injects its deployment key and the same repository contract on both platforms.

`POST /api/projects/{id}/machine` accepts strict, bounded preview/apply/approve commands. Only apply invokes the provider, and requires an explicit confirmation, provider identity and expected project version. Previews read local state and the adapter's offline language catalogue. Status exposes only provider identity, configured/supported booleans and counts. Secrets never enter project state, UI props, audit or revisions.

Selection is re-derived server-side from Base Language values and blank target values; submitted IDs cannot overwrite existing translations. Version checks precede and follow the external call, with the repository's atomic compare-and-swap as the final authority. External work cannot be rolled back: stale results are discarded even if the provider has billed the request. Independent invalid items may be omitted; all valid items, provenance, review flags, timestamps, audit, beforeMachineTranslation/afterMachineTranslation checkpoints and retention commit together. Transport, batch or malformed-response failure commits nothing. All-invalid results do not create empty history.

Shielding distinguishes each occurrence of `{name}` and `{{name}}`, protects generated-token-like user text and XML-incompatible characters, and verifies exact token multiplicity before restoring once. The adapter encodes tokens as ignored XML elements with IDs, escapes literal markup and checks returned root IDs. Existing resource-format validation remains authoritative after restoration.

Translations add nullable `originProvider` and `originModel`; only provider-reported meaningful models are stored. SQLite migration 4 and D1 migration 0002 extend their released schemas without editing history. Old checkpoints can omit new metadata and restore with nulls. Manual value changes clear metadata, unchanged machine approval preserves it, and Base Language review invalidation retains provenance. Explicit per-entry/bulk approval records audit and lifecycle changes without checkpoints. No imports, reads, previews, exports, approvals or restores cause provider requests.
